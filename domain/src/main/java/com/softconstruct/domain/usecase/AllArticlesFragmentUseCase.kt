...
import com.softconstruct.domain.extensions.fromResponseToRoomModel
...
internal class AllArticlesFragmentUseCase(private val allArticlesFragmentRepository: AllArticlesFragmentRepository) :
    AllArticlesFragmentInteractor {
            ...
            is com.softconstruct.entity.Result.Success -> {
                ...
                result.data?.response?.results?.map {
                    it.fromResponseToRoomModel(
                        favoriteArticlesIdFromDB.contains(it.id)
                    )
                }?.let {
